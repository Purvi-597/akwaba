...
if(mapurl == 'http://103.244.122.110:5000/mapproject/akwaba/index.php') {
var plainMap = L.tileLayer('http://103.244.122.110:5000/hot/{z}/{x}/{y}.png', {
    maxZoom: 21,
    maxNativeZoom: 22
});
var base_url = 'http://103.244.122.110:5000/mapproject/akwaba/';
}else{
var plainMap = L.tileLayer('http://10.10.3.50:5000/hot/{z}/{x}/{y}.png', {
    maxZoom: 21,
    maxNativeZoom: 22
});	
var base_url = 'http://10.10.3.50:5000/mapproject/akwaba/';	
}
...
$(document).on('click','.photos', function(){ 
	...
	for (var index = 0; index < touristLatLong.length; index++) {
		let customIcon = {
			iconUrl: '../public/uploads/photos/'+touristLatLong[index]['coordinates'][2],
			iconSize:[30,30]
		}
...
$(document).on('click','#catBtn', function(){
	...
	$.ajax({
		...
		success: function(response){
			...
			$("#mcloseBtn").addClass('morecloseBtn');
			$('#mcloseBtn').removeAttr('id');
...
$(document).on('click','.morecloseBtn', function(){
	$(".morecategoryDiv").removeClass('extralarge');
	$(".subcatSubsidebar").css('display','none');
	$('.morecloseBtn').attr('id', 'mcloseBtn');
});
